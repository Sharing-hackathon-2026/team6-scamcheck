...
- **Backend:** Flask 3.x, Jinja2 template, `requests` gọi Gemini REST (`generativelanguage.googleapis.com`).
...
- **Triển khai:** systemd trên VM exe.dev (port 9595) + git. *(Đề bài cho Render/Railway/GitHub Pages; ở đây chạy trên VM riêng nên dùng systemd + reverse proxy exe.dev — tương đương "địa chỉ web công khai" ở Cấp 1 L1-05.)*
...
- [ ] **L1-02** Giao diện nhập liệu cơ bản — route `/`, `<textarea>` lớn + nút **Kiểm tra**, footer pháp lý. Chưa cần đẹp.
- [ ] **L1-03** Tích hợp Gemini — route `POST /check` gọi `services/gemini.py:generate_text()`, trả về **văn bản thô**.
- [ ] **L1-04** Dòng cảnh báo pháp lý — partial `_legal.html` ở mọi template.
- [ ] **L1-05** Triển khai lên mạng — systemd + proxy exe.dev, xác nhận địa chỉ công khai.

**Test (pytest):**
- `tests/test_gemini_raw.py` — mock HTTP, verify request payload đúng (key, model, prompt).
- `tests/test_routes.py::test_home_has_legal_footer`.
...
> Đóng băng foundation tokens (font ≥18px, spacing, màu) vào `app/static/css/tokens.css`.
...
- [ ] **L2-01** Hồ sơ & system prompt Thám tử (giọng khô khan, lý tính) + **cấu trúc JSON cố định**:
  ...
  ```
- [ ] **L2-02** Hàm đọc kết quả **có dự phòng** — `services/parser.py:parse_detective()`: validate schema, coerce, fallback mặc định an toàn khi lệch. **Bắt buộc test 5 trường hợp lệch.**
- [ ] **L2-03** Thẻ màu mức rủi ro (component `_risk_card.html`).
- [ ] **L2-04** Danh sách dấu hiệu + **tô vàng đoạn trích** trong tin gốc (JS `highlight-excerpts.js`: tìm chuỗi con, bọc `<mark>`; không tìm thấy thì bỏ qua).
...
- [ ] **L2-09** Lịch sử 10 tin gần nhất — **localStorage** (`history.js`), xem lại không gọi AI lại.
- [ ] **L2-10** Giao diện thân thiện iPhone — font ≥18px, tương phản AA, layout không vỡ.

**Test (pytest):**
- `tests/test_parser_detective.py` — 5 case lệch cấu trúc + case đúng.
- `tests/test_validate_input.py` — rỗng, >5000 ký tự.
- `tests/test_gemini_structured.py` — mock verify `response_mime_type=application/json`.
...
**Test (pytest):**
- `tests/test_psychologist_chain.py` — chuỗi tuần tự đúng thứ tự, mock AI thứ 2 ném lỗi → Thám tử vẫn trả được.
- `tests/test_activation_condition.py` — `an_toan` không trigger gọi thứ 2 (verify số lần gọi Gemini = 1).
...
- [ ] **L4-B1** Tách đường dẫn (`services/links.py:extract_urls()`) — regex, bao gồm link rút gọn (bit.ly, ...). Test không sót, không nhầm text thường.
...
- [ ] **L4-C2** Route `/practice` — hiện từng câu, user đoán, chấm + giải thích ngay, tổng kết điểm + nhận xét.
- [ ] Điều hướng giữa trang chính ↔ luyện tập **không reload full app** (fetch/SPA nhẹ hoặc route riêng).

**Test (pytest):**
- `tests/test_links.py` — extract + detect, ≥5 case giả mạo.
- `tests/test_quiz.py` — tải data đúng, tính điểm đúng.
...
**Test (pytest):**
- `tests/test_rescuer.py` — 4 kịch bản, post-filter số lạ = rỗng.
- `tests/test_hotlines_filter.py` — số trong whitelist giữ nguyên, số lạ bị strip.
...
|---|---|---|
| Stack | Flask + Jinja2 + Tailwind CDN | Theo khuyến nghị đề bài, nhóm đã học Python |
...
| Triển khai | systemd + reverse proxy exe.dev (port 9595) | VM công khai, tương đương "địa chỉ web công khai" |
