...
- **Model Gemini:** `gemini-3.5-flash-lite` qua endpoint `generateContent`, yêu cầu **JSON có cấu trúc** (`response_mime_type=application/json`) để parse deterministic.
...
- [x] `requests` + verify key thật `gemini-3.5-flash-lite` gọi OK.
...
**Test Stage 2:** `77` pytest backend + `21` Node tests frontend; compileall, node syntax, `git diff --check` đều đạt. Smoke test Gemini thật xác nhận model `gemini-3.5-flash-lite` chấp nhận schema tương thích và trả đúng contract.
...
| L5-10 | Tương phản cao + cỡ chữ điều chỉnh | Bắt buộc | ✅ Xong | High contrast light/dark + dropdown 100/115/130% trên bốn trang, smooth resize và localStorage fail-safe |

**Test hiện tại:** 205 pytest backend + 83 Node tests frontend; compileall/syntax/NFC/diff check đạt.
